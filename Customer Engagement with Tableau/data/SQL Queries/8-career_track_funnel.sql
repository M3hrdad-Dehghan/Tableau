...
table_course_exams as
(SELECT DISTINCT se.student_id, e.course_id 
...
(SELECT  student_id, enrolled_in_track_id,  MAX(attempted_course_exam) AS attempted_course_exam, MAX(certificate_course_id) as certificate_course_id
FROM SELECT 
    c.*,
    CASE
